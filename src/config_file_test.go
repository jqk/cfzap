...
func TestReadConfigFile(t *testing.T) {
	...
	_, err = readConfigFile(NewConfigOption(WithFileName(""), WithFileExt(""), WithFilePaths(TEST_FILE_PATH)))
	assert.Nil(t, err, "fail to read config file in default type")

	// test read config file with default name and json type.
	// using cfzap.json as the config file.
	_, err = readConfigFile(NewConfigOption(WithFileName(""), WithFileExt("json"), WithFilePaths(TEST_FILE_PATH)))
	...
	_, err = readConfigFile(NewConfigOption(WithFileName("configfile"), WithFileExt("yaml"), WithFilePaths(TEST_FILE_PATH)))
